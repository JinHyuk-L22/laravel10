...
CKEDITOR.plugins.add("wordcount", {
    ...
    init: function (editor) {
        ...
        function updateCounter(editorInstance) {
            ...
            //data1 = countWords(CKEDITOR.instances.abs_title.getData());
            data2 = countWords(CKEDITOR.instances.abstract.getData());
            data3 = countWords(CKEDITOR.instances.abstract2.getData());
            data4 = countWords(CKEDITOR.instances.abstract3.getData());
            data5 = countWords(CKEDITOR.instances.abstract4.getData());

            if($("#insert_type").is(":checked")==true){
                if(data5>400) new_data1 = "<span style='color:#ff0000; font-weight:bold;'>" + data5 + "</span>";
                else new_data1 = data5;

                //$("#remLen_max2").val(data5);
                //$("#remLen2").html(new_data1);
                $("#word_count").val(new_data1);
            }else{

                //$("#words_cnt0").val(data1);
                $("#words_cnt1").val(data2);
                $("#words_cnt2").val(data3);
                $("#words_cnt3").val(data4);
                $("#words_cnt4").val(data5);

                var tot_data = data1+data2+data3+data4+data5;

                if(tot_data > 300){
                    alert('최대 입력 가능 단어 수는 300 단어를 넘지 못합니다.');
                    new_data2 = tot_data;
                } else{
                    new_data2 = tot_data;
                }
                ...
                $("input[name=abs_cnt]").val(new_data2);
